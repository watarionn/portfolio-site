...
assert.equal(projectCount.textContent, '18');
assert.equal(districtCount.textContent, '4');
assert.equal(visitedCount.textContent, '0');
assert.equal(visitedTotal.textContent, '18');
...
assert.equal(buildings.length, 18, 'map must render eighteen buildings');
...
assert.equal(buildings.filter((button) => button.dataset.worldAnchor === 'bottom-center').length, 18, 'every building must expose world placement data');
assert.equal(documentMock.querySelectorAll('.building-visual').length, 18, 'every building must render a handcrafted visual');
...
assert.equal(workRows.length, 18, 'works directory must render eighteen rows');
...
console.log('Portfolio City Phase 3.6 R5 runtime contract passed: 13 chunks / 4 districts / 18 world placements / negative origin / navigation / keyboard / visited state');

...
def main() -> int:
    ...
    for marker in ("portfolio-city.visited.v1", "localStorage", "ArrowRight", "ArrowLeft", "replaceChildren", "prefers-reduced-motion", "buildBuildingVisual", "buildMapInfrastructure", "buildDistrictLandmark", "buildDistrictScene", "is-active-district", "district__progress", "city-map__street-label"):
        if marker not in js:
    ...
    for marker in ("@media (max-width: 680px)", "@media (prefers-reduced-motion: reduce)", ".district--north", ".district--south", ".city-map__road--spine", ".city-map__bridge", ".district-landmark", ".district-scene", ".is-active-district", ".city-map__street-label", ".district__progress"):

...
assert.equal(projectCount.textContent, '17');
assert.equal(districtCount.textContent, '4');
assert.equal(visitedCount.textContent, '0');
assert.equal(visitedTotal.textContent, '17');
...
assert.equal(buildings.length, 17, 'map must render seventeen buildings');
...
assert.equal(cityMap.dataset.worldMinY, '-768');
assert.equal(cityMap.dataset.worldWidth, '5120');
assert.equal(cityMap.dataset.worldHeight, '3840');
assert.equal(cityMap.dataset.worldMaxX, '4096');
assert.equal(cityMap.dataset.worldMaxY, '3072');
assert.equal(cityMap.dataset.terrainMode, 'full', '13 illustrated chunks must activate full terrain mode');
const observatory = districts.find((district) => district.dataset.districtId === 'observatory-hill');
assert.ok(observatory, 'Observatory district must render');
assert.equal(observatory.style.getPropertyValue('--district-left'), '35%');
assert.equal(observatory.style.getPropertyValue('--district-width'), '32.5%');
const holoscope = buildings.find((button) => button.dataset.projectId === 'holoscope');
assert.ok(holoscope, 'HoloScope building must render');
assert.equal(holoscope.style.getPropertyValue('--project-left'), '50%');
...
assert.equal(currentIllustrated.style.getPropertyValue('--chunk-left'), '40%');
...
assert.equal(buildings.filter((button) => button.dataset.worldAnchor === 'bottom-center').length, 17, 'every building must expose world placement data');
assert.equal(documentMock.querySelectorAll('.building-visual').length, 17, 'every building must render a handcrafted visual');
...
assert.equal(workRows.length, 17, 'works directory must render seventeen rows');
...
console.log('Portfolio City Phase 3.6 R5 runtime contract passed: 13 chunks / 4 districts / 17 world placements / negative origin / navigation / keyboard / visited state');

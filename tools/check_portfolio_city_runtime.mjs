...
assert.equal(projectCount.textContent, '16');
assert.equal(districtCount.textContent, '4');
assert.equal(visitedCount.textContent, '0');
assert.equal(visitedTotal.textContent, '16');
...
assert.equal(buildings.length, 16, 'map must render sixteen buildings');
...
assert.equal(buildings.filter((button) => button.dataset.worldAnchor === 'bottom-center').length, 16, 'every building must expose world placement data');
assert.equal(documentMock.querySelectorAll('.building-visual').length, 16, 'every building must render a handcrafted visual');
...
assert.equal(workRows.length, 16, 'works directory must render sixteen rows');
...
console.log('Portfolio City Phase 3.6 R5 runtime contract passed: 13 chunks / 4 districts / 16 world placements / negative origin / navigation / keyboard / visited state');
